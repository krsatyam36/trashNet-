...
Original file is located at
    https://colab.research.google.com/drive/1QjoDnGpnwP6nMQrv-ZSkdOjjaIpKHTRG
"""

# =============================
# Colab Cell 1 — Setup & Hugging Face login
# =============================

# Install required libraries:
# - datasets: load datasets from Hugging Face Hub
# - huggingface_hub: interact with Hugging Face Hub (login, download models, etc.)
# - tqdm: progress bars for loops
# - scikit-learn: evaluation metrics like precision, recall, f1-score
# - onnxruntime: run inference with ONNX models
!pip install -q datasets huggingface_hub tqdm scikit-learn onnxruntime

# (Optional) If torch or torchvision is missing, you can install them explicitly:
# !pip install -q torch torchvision

# Import the Hugging Face login function
from huggingface_hub import login

# Directly log in using Hugging Face token
# Normally we would use getpass() for hidden input, but here we hardcode the token.
hf_token =

# Perform login with the token
...
EPOCHS = 2                  # number of training epochs (adjustable) #keeping 2 as i don't have colab cerdits right now
...
# -----------------------------
torchscript_path = "/content/models/resnet18_scrap_scripted.pt"
onnx_path = "/content/models/resnet18_scrap.onnx"
...
# -----------------------------
os.makedirs("/content/results", exist_ok=True)                 # create results directory if not present
csv_path = "/content/results/predictions.csv"                  # CSV file to log predictions
...
# -----------------------------
# Run simulation and log predictions
# -----------------------------
with open(csv_path, "w", newline="") as f:
    ...
    # loop through all images in the test split
    for i, ex in enumerate(dataset_splits["test"]):
        # load image
        img = ex["image"]
        if not isinstance(img, Image.Image):
            img = Image.fromarray(img)
...
# Simulation complete
# -----------------------------
print("Simulation complete. Predictions saved to:", csv_path)

# =============================
# Colab Cell 7b — Stage 4: Conveyor Visualization with Predictions
# =============================
...
# -----------------------------
#  switch between TorchScript or ONNX depending on which export you want to test.
inference_fn = predict_torchscript  # or predict_onnx

# -----------------------------
# Simulation parameters
# -----------------------------
CONF_THRESHOLD = 0.60    # flag predictions below this probability
FRAME_INTERVAL = 0.5     # delay between frames (in seconds) for visualization

# -----------------------------
# Loop over first 20 test samples for demo visualization
# -----------------------------
for i in range(20):
    # get sample from test split
    ...
    # prepare text with class + confidence
    conf_text = f"{pred_class} ({conf*100:.1f}%)"
    if conf < CONF_THRESHOLD:
        conf_text += " LOW CONFIDENCE"   # append warning if below threshold

    # -----------------------------
    # Visualization
    # -----------------------------
    plt.imshow(img)          # show the image
    plt.title(conf_text)     # add prediction as title
    plt.axis("off")          # hide axis for cleaner look
    plt.show()

    # add a pause to simulate real-time frame capture
    time.sleep(FRAME_INTERVAL)
...
# -----------------------------
checkpoint = torch.load("/content/models/resnet18_best.pth", map_location=DEVICE)
...
cm_path = "/content/results/confusion_matrix.png"
plt.savefig(cm_path)
plt.show()
